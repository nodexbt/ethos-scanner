...
interface DecryptedTextProps {
  ...
  /** If set, automatically replays the decrypt animation every N milliseconds. */
...
export default function DecryptedText({
  ...
  // Periodic replay: re-runs the decrypt animation on a timer.
  useEffect(() => {
    if (!replayInterval || replayInterval <= 0) return;
    const id = setInterval(() => {
      // Reset state and kick off a fresh forward animation, just like a hover.
      setIsAnimating(false);
      setRevealedIndices(new Set());
      setIsDecrypted(false);
      setDisplayText(text);
      setDirection("forward");
      // Defer the start so React commits the reset before the interval tick.
